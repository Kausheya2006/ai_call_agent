...
async def generateChat(
    ...
    try:
        logging.info(f"Generating AI response for: {messages}")
        formatted_messages = [
            {
...
async def websocket_handler(websocket):
    try:
        await websocket.send("Hello! WebSocket connection established.")
        async for message in websocket:
...
# Run aiohttp in the Main Event Loop
async def run_aiohttp():
    runner = web.AppRunner(app)
    await runner.setup()
    site = web.TCPSite(runner, '0.0.0.0', 5001)
    await site.start()
    logging.info("Aiohttp server started on http://0.0.0.0:5001")
    await asyncio.Future()  # Run forever

# Main Coroutine to Start All Servers
async def main():
    # Start aiohttp and WebSocket server in the asyncio event loop
    aiohttp_task = asyncio.create_task(run_aiohttp())
    ...
    # Wait for aiohttp and WebSocket tasks
    await asyncio.gather(aiohttp_task, websocket_task)
